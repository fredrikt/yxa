...
%%           (gregorian seconds in UTC format), Type is ONLY used to 
...
%% Note    : "If neither "tzid" nor "tzurl" [Timezone] are present, 
%%            all non-UTC times within this time switch should be 
%%            interpreted as being "floating" times, i.e., that they
%%            are specified in the local time-zone of the CPL server."
%%           - RFC 3880 chapter 4.4 p15
%%           * tzid and tzurl are currently unsupported              
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% Function: add_datetime_and_duration(DateTime, Duration)
%%           DateTime = date_time record()
%%           Duration = duration record()
%% Descrip.: addition is done in a "natural" way, e.g.:
%% 
...
%%           of passed seconds.
%%           Note that each second counts as a range of Duration = 1
%%           second, rather than a point in time, this implies:
%%            
%%           DateTime = 2005-01-01 00:00:00 and Duration = 1 second 
%%           -> range = [2005-01-01 00:00:00, 2005-01-01 00:00:00] 
%%
%% Returns : date_time record()
%% Note    : duration record() don't handle years and month so the 
%%           date part of a date can never become illegal, but the time
%%           part may become so due to change to DST  
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% Function: add_xxx(DateTime, xxxTime)
%%           DateTime = date_time record()
%%           xxxTime  = integer()
...
%% Returns : date_time record()
%% Note    : This code expects DateTime to contain a legal date and 
%%           time value, so no skipped - transition to DST hour or 
