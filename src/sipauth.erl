...
do_get_user_verified2(Method, User, OrigUser, Password, Realm, Now, AuthDict) ->
	...
	AuthTimeValid < 0 ->
	    ...
	    {stale, User};
	Timestamp > (Now + 5) ->
	    %% tolerate timestamps slightly into the future in case the challenge was generated by
	    %% another proxy in a server farm, and the clocks of the servers are not perfectly in sync
...
test() ->
    ...
    autotest:mark(?LINE, "do_get_user_verified2/7 - 2.1"),
    %% Correct response, time in the _near_ future
    {authenticated, "canon-user"} =
	do_get_user_verified2(AuthMethod1, "canon-user", AuthUser1, AuthPassword1,
			      AuthRealm1, AuthNow1 - 2, AuthDict1),

    autotest:mark(?LINE, "do_get_user_verified2/7 - 2.2"),
    %% Correct response, time in the future
    false =
	do_get_user_verified2(AuthMethod1, "canon-user", AuthUser1, AuthPassword1,
			      AuthRealm1, AuthNow1 - 12, AuthDict1),
